{ $Id: TestModules.pas,v 1.1 2001/07/05 13:02:47 chrismo Exp $ }
...
const
  rcs_id :string = '#(@)$Id: TestModules.pas,v 1.1 2001/07/05 13:02:47 chrismo Exp $';
...
begin
  ...
  LibHandle := 0;
  if not LoadModule(LibHandle, LibName) then
  ...
  begin
    ...
    begin
      U := GetTest;
      if U = nil then
        raise EDUnitException.Create(Format('Module "%s.Test" returned nil', [LibName]))
      else if 0 <> U.QueryInterface(ITest, Result) then
